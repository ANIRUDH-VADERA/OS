...
int main()
{
    int n, i, m, arrival[100], burst[100], min, already_exec[100], exec_idx = 0, flag[100], temp_arr[100], temp_idx = 0, real_min, waiting[100], sum1 = 0, remaining_time = 0, total_remaining_time = 0, temp_arr2[100], temp_idx2 = 0, temp_arr3[100], temp_idx3 = 0, f = 0, k = 0, j = 0, temp = 0;
    float average_wait;
    int min2 = 0;
    scanf("%d", &n);
    for (i = 0; i < n; i++)
    ...
    printf("real_min : %d\n", real_min);
    printf("min : %d\n", min);
    if ((burst[min] - arrival[real_min] + arrival[min]) <= burst[real_min])
    {
        real_min = min;
    }
    f = burst[min];
        ...
        for (i = arrival[min] + 1; i < arrival[real_min] + 1; i++)
        {
            for (m = 0; m < temp_idx; m++)
            {
                if (arrival[temp_arr[m]] == i && burst[temp_arr[m]] < f)
                {
                    if (k == 0)
                    {
                        burst[min] = burst[min] - (arrival[temp_arr[m]] - burst_total(already_exec, exec_idx, burst, total_remaining_time));
                    ...
                    {
                        burst[temp] = burst[temp] - (arrival[temp_arr[m]] - burst_total(already_exec, exec_idx, burst, total_remaining_time));
